...
class Point {
  ...
  constructor(x, y, radius=null, pinned=false) {
    this.radius = radius ?? randomInt(MIN_SIZE, MAX_SIZE);
    this.mass = this.radius * RATIO_MASS_OVER_RADIUS;
    this.x = x;
    this.y = y;
    this.last_x = x + (Math.random() - 0.5); // random initial motion
    this.last_y = y + (Math.random() - 0.5);
    this.color = colors[randomInt(0, colors.length - 1)]; // color "red" if pinned
    this.pinned = pinned;
  }
  ...
  render(ctx) {
    ctx.beginPath();
    ctx.fillStyle = this.pinned ? 'red' : this.color;
...
function collide(pt1, pt2) {
  return Math.sqrt(Math.pow(pt1.x - pt2.x, 2) + Math.pow(pt1.y - pt2.y, 2)) < pt1.radius + pt2.radius;
}
...
function solveCollision(pt1, pt2) { // first version generated by ChatGPT ^^
  ...
  pt1.last_x += nx * overlap;
  pt1.last_y += ny * overlap;
  pt2.last_x -= nx * overlap;
  pt2.last_y -= ny * overlap;
...
function step(timestamp) {
  ...
  const dt = Math.min(25 / 1000, (timestamp - lastTimestamp) / 1000); // sec.
...
function main() {
  ...
  points = [
    // X, Y
    new Point(275, 75, SIZE), // upper left
    new Point(325, 75, SIZE), // upper right
    new Point(325, 125, SIZE), // lower right
    new Point(275, 125, SIZE), // lower left
    new Point(300, 25, SIZE, true), // pinned anchor
    new Point(300, 50, SIZE), // chain
    new Point(300, 80, SIZE), // chain
    // CHAIN 2
    new Point(100, 100, 10, true), // pinned anchor
    new Point(150, 200, 10),
    new Point(200, 200, 10),
    new Point(250, 200, 10),
    new Point(300, 200, 10),
    new Point(350, 200, 10),
    new Point(400, 200, 10),
    new Point(450, 200, 10),
    new Point(canvas.width - 100, 100, SIZE, true), // pinned anchor
  ];
