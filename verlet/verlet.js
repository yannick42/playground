...
function solveCollision(pt1, pt2) { // first version generated by ChatGPT ^^
  ...
  const ratio1 = pt1.mass / totalMass; // pt1 moves this much
  const ratio2 = pt2.mass / totalMass; // pt2 moves this much
  ...
  if (! pt1.pinned) {
    const vel1x = pt1.x - pt1.last_x;
    const vel1y = pt1.y - pt1.last_y;
  ...
  if (! pt2.pinned) {
    const vel2x = pt2.x - pt2.last_x;
    const vel2y = pt2.y - pt2.last_y;
...
function main() {
  ...
  const NB_I = 24; // number of points in the 2nd chain
